import { Avatar, Box, Container, Flex, SimpleGrid, Skeleton, Stack, Text } from "@chakra-ui/react";
import { MediaRenderer, ThirdwebNftMedia, useContract, useMinimumNextBid, useValidDirectListings, useValidEnglishAuctions } from "@thirdweb-dev/react";
import { NFT, ThirdwebSDK } from "@thirdweb-dev/sdk";
import React, { useState } from "react";
import { 
    MARKETPLACE_ADDRESS,
    NFT2_COLLECTION_ADDRESS 
...
export default function TokenPage({ nft, contractMetadata }: Props) {
    ...
    const { contract: nftCollection } = useContract(NFT2_COLLECTION_ADDRESS);

    const { data: directListing, isLoading: loadingDirectListing } = 
        useValidDirectListings(marketplace, {
            tokenContract: NFT2_COLLECTION_ADDRESS, 
    ...
    async function createBidOffer() {
        ...
        if (auctionListing?.[0]) {
            ...
            txResult = await marketplace?.offers.makeOffer({
                assetContractAddress: NFT2_COLLECTION_ADDRESS,
                tokenId: nft.metadata.id,
                totalPrice: bidValue,
            })
        } else {
...
export const getStaticProps: GetStaticProps = async (context) => {
    ...
    const contract = await sdk.getContract(NFT2_COLLECTION_ADDRESS);
  ...
  export const getStaticPaths: GetStaticPaths = async () => {
    ...
    const paths = nfts.map((nft) => {
      return {
        params: {
          contractAddress: NFT2_COLLECTION_ADDRESS,
          tokenId: nft.metadata.id,
        },
      };
    });
